...
*/div.myc{
	...
	background:#4d0000;padding:10px;
	text-align:center;
...
<div class="w3-overlay-white w3-animate-opacity" onclick="w3_close()" style="cursor:pointer" id="myOverlay"></div>
  ...
  style="font-size: 15px;color: white;" onclick="w3_open()"><i class="fa fa-bars"></i></a>
     ...
     <br>
     <br>   <h2 class="w3-center w3-text-light-green"><b><u>Normal Property</u></b></h2>
...
<%try{
	...
	String sql="select * from PROPERTY where PRICE<=? order by PRICE";
...
%>
 ...
 <a href="Propertyview.jsp?rd=<%=rs.getString(2)%>"><img src="<%=rs.getString(9)%>" height="200px" width="280px"><br><%=rs.getString(2) %><br><%=rs.getString(7)%><br><%=z%>lakhs</a>
</div>
